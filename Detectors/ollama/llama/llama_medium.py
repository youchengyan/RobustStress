...
def call(prompt):
    try:
        client = OpenAI(
            ...
            api_key="ollama"  # Ollama 接受任意非空 key
        )

        # 调用时使用 exact model name from `ollama list`
        response = client.chat.completions.create(
            model="llama3:8b",  # ← 关键：去掉 -q4
            messages=[
    ...
    for i in range(start_index, total):
        ...
        try:
            if original_text.strip():
                prompt = (
                        "Paraphrase this text while preserving the original meaning precisely, but feel free to rephrase clauses or use different expressions where appropriate. Output ONLY the paraphrased text, with no introductory sentences. The following text: " + original_text + "Paraphrased version:"
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default="qwen2.5:7b-instruct", help="Ollama 模型名称")
    parser.add_argument("--save_every", type=int, default=5, help="每处理 N 条保存一次")
