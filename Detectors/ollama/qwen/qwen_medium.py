...
def call(prompt):
    try:
        ...
        response = client.chat.completions.create(
            model="qwen2.5:7b-instruct",  # ← 关键：去掉 -q4
            messages=[
    ...
    for i in range(start_index, total):
        ...
        try:
            if original_text.strip():
                prompt = (
                        "Paraphrase this text while preserving the original meaning precisely, but feel free to rephrase clauses or use different expressions where appropriate. The following text: " + original_text
                )
