    ...
    with tqdm(loader, desc=desc, disable=distributed() and dist.get_rank() > 0) as loop:
        for texts, masks, labels in loop:
            ...
            if args.lamb > 0:  # PU loss activated, self added
                ...
                U_mask = (sentence_length < args.len_thres) & (
                    labels.bool())  # select short, chatgpt sentences as unlabeled
            ...
            if not (distributed() and dist.get_rank() > 0) and args.count_iter % 100 == 0:  # self added: print to log
                print(
                    f'{desc} iter {args.count_iter}; Loss={loss.item():.3f}; Acc={(train_accuracy / train_epoch_size):.3f}')
...
def validate(model: nn.Module, device: str, loader: DataLoader, votes=1, desc='Validation', args=None):
    ...
    all_logits = torch.cat(all_logits, dim=0).float()  # [N]
    all_labels = torch.cat(all_labels, dim=0).long()  # [N]

    probs = torch.softmax(all_logits, dim=1)  # [N, 2]
    ...
    # 获取基础文件名（不含扩展名）
    basename = os.path.basename(args.val_file1)  # e.g., "test.json"
    file_name_without_ext = os.path.splitext(basename)[0]

    # 构造输出目录和日志文件路径
    output_dir = args.output_dir
    os.makedirs(output_dir, exist_ok=True)
    log_path = os.path.join(output_dir, f"{file_name_without_ext}_MPU_auroc.txt")

    # 构造日志行
    log_line = f'auroc: {auroc:.4f};'
...
def brief_validate(model, device, validation_loader, epoch, rank, val_name='', args=None):
    ...
    try:
        accuracy = (combined_metrics['valTP'] + combined_metrics['valTN']) / (
                    combined_metrics['valTP'] + combined_metrics['valTN'] + combined_metrics['valFN'] +
                    combined_metrics['valFP'])
    ...
    if rank == 0:
        ...
        output_dir = args.output_dir
        os.makedirs(output_dir, exist_ok=True)
        log_path = os.path.join(output_dir, f"{file_name_without_ext}_MPU_metrics.txt")

        # 构造日志行
        log_line = f'Val {val_name} Epoch {epoch}== Accuracy: {accuracy:.4f}; Precision: {precision:.4f}; Recall: {recall:.4f}; F1Score: {f1:.4f}.\n'
        ...
        print(
            f'Val {val_name} Epoch {epoch}== Accuracy: {accuracy:.4f}; Precision: {precision:.4f}; Recall: {recall:.4f}; F1Score: {f1:.4f}.')
...
def run(max_epochs=None,
    ...
    train_loader, validation_loader, validation_loader1, validation_loader2, validation_loader3, validation_loader4, validation_loader5, validation_loader6 = chatgpt_load_datasets(
        kwargs['train_data_file'], kwargs['val_data_file'], tokenizer, batch_size,
        max_sequence_length, random_sequence_length, epoch_size,
        token_dropout, seed, mode=kwargs['mode'], val_file1=kwargs['val_file1'], val_file2=kwargs['val_file2'],
        val_file3=kwargs['val_file3'], val_file4=kwargs['val_file4'], val_file5=kwargs['val_file5'],
        val_file6=kwargs['val_file6'], args=kwargs['args'])
    ...
    kwargs['args'].total_iter = None if kwargs['args'].training_proportion is None else round(
        len(train_loader) * kwargs['args'].training_proportion)  # training_proportion None: disable!
    ...
    for epoch in epoch_loop:
        ...
        if kwargs['args'].quick_val:  # self added quick_val
            kwargs['args'].TPFNTNFP = [combined_metrics['valTP'], combined_metrics['valFN'], combined_metrics['valTN'],
                                       combined_metrics['valFP']]
        try:
            accuracy = (combined_metrics['valTP'] + combined_metrics['valTN']) / (
                        combined_metrics['valTP'] + combined_metrics['valTN'] + combined_metrics['valFN'] +
        ...
        if rank == 0:
            # print(f'222222222222222222222222')
            print(
                f'Epoch {epoch}== Precision: {precision:.4f}; Recall: {recall:.4f}; F1Score: {f1:.4f}; Accuracy: {accuracy:.4f}; Best F1 {best_f1:.4f} @ep{best_f1_epoch}.')
...
# CUDA_VISIBLE_DEVICES=2 python train.py --batch-size 32 --max-sequence-length 512 --train-data-file /data5/storage4/wudauser05/yyc/RobustStressBench/domain_specific_model_specific/xsum/train.json --val-data-file /data5/storage4/wudauser05/yyc/RobustStressBench/domain_specific_model_specific/xsum/test.json --lamb 0.4 --prior 0.2 --pu_type dual_softmax_dyn_dtrun --len_thres 55 --aug_min_length 1 --max-epochs 1 --weight-decay 0 --mode tweep --aug_mode sentence_deletion-0.25 --clean 1 --quick_val 1 --learning-rate 5e-05 --seed 2026
...
if __name__ == '__main__':
    ...
    if args.log_dir is None:  # train_summary/train_config/Aug+PUconfig
        ...
        basename = os.path.basename(args.val_file1)  # → "test.json"
        file_name_without_ext = os.path.splitext(basename)[0]

        args.log_dir = f'./results/{file_name_without_ext}'
