...
def get_log_probs(texts, args, base_tokenizer, base_model):
    ...
    for batch in range(len(texts) // batch_size):
        tokenized = base_tokenizer(texts[batch * batch_size:(batch + 1) * batch_size], return_tensors="pt", truncation=True, max_length=2048,
...
def _sample_from_model(texts, args, base_tokenizer, base_model, truncate_ratio=0.5):
    ...
    all_encoded = base_tokenizer(texts, return_tensors="pt", truncation=True, max_length=2048).to(args.DEVICE)
    ...
    while m < 100:
        ...
        min_length = 150
        outputs = base_model.generate(**all_encoded, min_length=400, max_new_tokens=800, do_sample=True,
...
def generate_samples(texts, args, base_tokenizer, base_model, batch_size):
    print(f'len(texts):{len(texts)},batch_size:{batch_size}')
...
def get_log_prob(text, args, base_tokenizer, base_model):
    ...
    tokenized = base_tokenizer(text, return_tensors="pt", padding=True, truncation=True, max_length=2048).to(args.DEVICE)
    labels = tokenized.input_ids[:, 1:]
    with torch.no_grad():
        logits_score = base_model(**tokenized).logits[:, :-1]
...
def experiment(args):
    ...
    regen_model = AutoModelForCausalLM.from_pretrained(
        args.regen_model,
        torch_dtype=torch.float16,  # 强烈建议用 FP16 节省显存
        low_cpu_mem_usage=True
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_model', default="/data5/storage4/wudauser05/yyc/gptneo", type=str, required=False)
    parser.add_argument('--regen_model', default="/data5/storage4/wudauser05/yyc/Meta-Llama-3-8B-Instruct", type=str, required=False)
    ...
    parser.add_argument('--regen_number', default=4, type=int, required=False)
