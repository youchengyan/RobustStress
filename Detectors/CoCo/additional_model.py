...
class GCNGraphAgg(nn.Module):
    ...
    def forward(
        ...
        L_new = sens_rep.size(1)

        # 创建与 sens_rep 长度匹配的新 mask（全 1）
        new_sen_mask = torch.ones(
            sens_rep.size(0),
            L_new,
            device=sens_rep.device,
            dtype=sentence_mask.dtype
        )

        final_rep, padded_output = self.rnn_coherence_proj(
            sens_rep, sentence_length, new_sen_mask
        )
        # print(f'final_rep:{final_rep}')
